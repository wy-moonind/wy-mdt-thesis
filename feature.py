...
def step2():
    model = torch.load('../show_feat_outer21_1_15_nlso.pt')
    model1 = torch.load('../show_feat_outer21_1_15_lso.pt')
    data_gen = MyData()
    train_set, test_set, show_set = data_gen.get_case_data("outer21")
    show_featmap = []
    show_featmap1 = []
    ...
    for idx, (batch_x, batch_obs, batch_y) in enumerate(show_loader):
        pred, feat_list = model(batch_x, y_obs=batch_obs)
        show_featmap.append(feat_list)
        pred1, feat_list1 = model1(batch_x, y_obs=batch_obs)
        show_featmap1.append(feat_list1)
    ...
    ax.imshow(show_featmap[0][0].cpu().detach())

    ax = fig.add_subplot(2, 1, 2)
    ax.set_title('Linear State Observer')
    ax.imshow(show_featmap1[0][0].cpu().detach())
...
def step3():
    ...
    for i in range(5):
        ...
        ax.imshow(show_featmap[0][i].cpu().detach())
    plt.xticks(size=10)
    plt.yticks(size=10)
    ...
    for j in range(5):
        ...
        ax.imshow(show_featmap1[0][j].cpu().detach())
...
def step5():
    model = torch.load('../models/outer21_15_1_serial_linear.pt')
    # outer21 test
    test_u = torch.load('../data/case_data_copy/test/test_u_outer21.pt')
    test_y = torch.load('../data/case_data_copy/test/test_y_outer21.pt')
    test_set1 = Data.TensorDataset(test_u.cuda(),
    ...
    plt.yticks(size=12)
    plt.xlabel('Timesteps',fontsize=15)
    plt.savefig('../figs/outer21_val_linear', dpi=300, bbox_inches='tight')
...
def step6():
    ...
    ax1.plot(epoch_vec, history[1, :], label="Training R2-value", color="darkorange")
    ax1.set_xlabel("Epoch", fontsize=12)
    ax1.set_ylabel("RMSE loss", fontsize=12)
    ax1.legend(loc=2)
    ax1.grid(axis="y")
    ax2 = ax1.twinx()
    ax2.plot(epoch_vec, history[2, :], label="Validation loss", color="green")
    ...
    plt.legend(['Prediction', 'Target'], loc="upper right", fontsize=12)
    plt.xticks(size=12)
    plt.yticks(size=12)
    plt.xlabel('Timesteps',fontsize=15)
