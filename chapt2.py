...
def femto_example():
    ...
    print(vib1.shape, vib3.shape, vib4.shape)

    vib = np.vstack((vib1, vib3, vib4))
    print(vib.shape)
    np.save("../fig_data/2.4", vib)
...
def fault_split():
    ...
    data_dict = scio.loadmat(path)
    data = data_dict.get('X246_DE_time')
    np.save("../fig_data/2.7", data[:10000])
...
def data_af_seg():
    ...
    datanp = np.array(data[0, :])
    print(type(datanp), datanp.shape)
    np.save("../fig_data/2.8", datanp)
